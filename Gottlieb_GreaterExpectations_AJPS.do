...
use "C:\Users\Sebastian\Google Drive\Education\Replication\Gottlieb 2016\survey.dta", clear
...
***Fixing for missing values in t2hi and t2low(SCG code) 
generate t2himv=.
generate t2lowmv=.
su indexi
replace t2himv=1 if t==2 & indexi>=`r(mean)'
replace t2lowmv=1 if t==2 & indexi<`r(mean)'

// Table 1 mv: Expectations index
qui xtmixed indexexpectnew t1 t2 i.enumerator i.block, ||cid: ||village:
estimates store E1
qui xtmixed indexexpectnew t1 t2himv i.enumerator i.block, ||cid: ||village:
estimates store E2mv
estout1 E1 E2mv, style(tex) replace star(0.05 0.01) b(%9.3fc) se(%9.3f par) conslbl("Intercept") label stats(N) stfmt(%9.0f) keep(t1 t2 t2hi _cons)  
...
// "Restitutions_AJPS", clear
use "C:\Users\Sebastian\Google Drive\Education\Replication\Gottlieb 2016\restitutions.dta", clear
...
merge cid using "C:\Users\Sebastian\Google Drive\Education\Replication\Gottlieb 2016\commune.dta"
table _merge
drop if _merge==2
